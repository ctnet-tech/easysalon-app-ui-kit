...
class SlidableTag extends StatelessWidget {
  const SlidableTag(
      {Key? key,
      this.data = const [],
      ...
      this.customTop,
      this.divideByPercent = const [], this.onClickEdit, this.onClickDelete})
      : super(key: key);
  ...
  final ValueChanged<String>? onClickEdit;
  final ValueChanged<String>? onClickDelete;
  // witget này da ta truyền vào vd : có  5 cột thì data truyền vào là 6 cột trong đó cột cuối cùng là key của cột đó để dễ dàng gọi onCkick và trả về key
  @override
  Widget build(BuildContext context) {
    ...
    var numberOfColumn = data.length;
    ...
    var listDataWidget = ({constraints}) => fixedListColumnData
        .map((indexColumn) => LayoutBuilder(builder: (context, containers) {
              var maxWidthCustom = constraints.maxWidth -
                  (layout.sizeToPadding(LayoutSize.small) * 2);
              return Column(
                children: [
                  // main row
                   Container(
                   height: 60,
                   child: (dataFistRow.length +1) == this.data[indexColumn].length ?  Slidable(
                     child: Container(
                     ...
                     secondaryActions: <Widget>[
                       IconSlideAction(
                         ...
                         onTap: () {
                           onClickEdit!(this.data[indexColumn].last);
                         },
                       ),
                       IconSlideAction(
                         caption: 'Xóa',
                         color: theme.getColor(ThemeColor.pattensBlue),
                         iconWidget:Icon(LineIcons.trash,color: theme.getColor(ThemeColor.secondary),) ,
                         onTap: () {
                           onClickDelete!(this.data[indexColumn].last);
                         },
                       ),
                     ],
                   ):Container(),
                ...
                child: Row(
                  children: fixedListDataFist
                      .map((indexRow) => dataRowWidget(
                          width: divideByPercentSet.length == 0
                              ? (maxWidthCustom / dataFistRow.length)
                              : (divideByPercentSet[indexRow] * maxWidthCustom),
                          isBold: fistRowBold,
